...
class Gate implements GateContract
{
    ...
    {
        /**
         * Gate提供的before方法允许你在一些特定情况下在执行其他检查前就返回，
         * 不再往下检查权限,官网的例子
         * 
    ...
    {//通过raw方法，获得当前操作的用户是谁。
        ...
        $arguments = is_array($arguments) ? $arguments : [$arguments];
           //callback前,从这里看出，如果BeforeCallBack返回true，将不会执行AuthCallback了
           //特别适合那些特殊权限的角色，如admin
        if (is_null($result = $this->callBeforeCallbacks($user, $ability, $arguments))) {
            $result = $this->callAuthCallback($user, $ability, $arguments);//callback
        }
        //callback后
     ...
     *  执行请求开始时AuthService里为所有的权限定义的回调函数，
     * @param  \Illuminate\Contracts\Auth\Authenticatable  $user
     * @param  string  $ability
     * @param  array  $arguments
     * @return callable
